...
import org.javaspec.matcher.MatchResult;
...
public class ObjectBehavior<T> {
    ...
    public void shouldBe(Object actual, Object expected) {
        MatchResult result = matcherRegistry.matcherFor("identity").evaluate(actual, expected);
        if (result.isFailed()) {
    ...
    public void shouldEqual(Object actual, Object expected) {
        MatchResult result = matcherRegistry.matcherFor("equality").evaluate(actual, expected);
        if (result.isFailed()) {
            throw new AssertionError(result.failureMessage());
        }
    }
    ...
    public void shouldNotBe(Object actual, Object unexpected) {
        MatchResult result = matcherRegistry.matcherFor("negated-identity").evaluate(actual, unexpected);
        if (result.isFailed()) {
            throw new AssertionError(result.failureMessage());
        }
    }
