...
- Approximate numeric matchers are implemented; iterable/iterator, dynamic object-state, and inline/configured matcher scope still require a 1.0 decision.
...
| P0-MATCH-001 | P0 | Dynamic object-state and iterator matcher contracts are not finalized. | Implement or explicitly defer/narrow scope before RC without contradicting the PHPSpec-first promise. |
