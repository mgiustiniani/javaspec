...
- Support zero-runtime-dependency interface doubles using JDK dynamic proxies, including argument matchers, throwing stubs, and answer callbacks, with explicit limitations for unsupported concrete/static/constructor/bytecode scenarios.
...
- Default CLI runs, the invocation API, and optional adapters do not compile source/spec files; source-only specs are skipped until compiled classes are available, with Phase 23 diagnostics to explain availability problems. Phase 29 adds CLI-only opt-in compilation through the current JDK `javax.tools.JavaCompiler`; optional Maven, Gradle, and JUnit Platform adapters supply host classpath integration but do not gain compilation behavior.
