...
| Deterministic CLI/build-tool/engine behavior | Commands, options, prompts, output modes, explicit classpath handling, bootstrap hook ordering/failure behavior, stronger interface double stub precedence/call recording behavior, stable ids/source metadata, report writing, Maven/Gradle plugin adapter behavior, JUnit Platform engine mapping, aggregate verification, release-readiness checks, and exit codes/events are stable for local and CI usage. | CLI and optional Maven/Gradle/JUnit Platform adapter behavior are documented in the user manual; Phase 12 ran 364 tests per JDK across the matrix; Phase 15 standalone Maven plugin verification passed with 12 plugin tests; Phase 16 standalone Gradle plugin verification passed with 11 plugin tests using Gradle 8.8; Phase 17 standalone JUnit Platform engine verification passed with 12 tests; Phase 18 core and adapter verification passed with stable id/source/report assertions; Phase 19 `scripts/verify-all.sh` full aggregate verification passed locally; Phase 20 version alignment, release-artifact packaging, and full aggregate verification passed locally; Phase 21 schema/golden validation, standalone examples verification, and full aggregate verification with examples passed locally; Phase 22 targeted changed tests, root `mvn -q test`, root `mvn -q verify`, standalone adapter verification, examples verification, and aggregate verification passed locally. Phase 27 bootstrap behavior is documented in ADR 0020 and user/architecture documentation; Phase 28 stronger interface doubles are documented in ADR 0021 and user/architecture documentation; this documentation handoff did not add test evidence. |
...
- Features that normally need libraries, such as configuration parsing, JSON/XML writing, doubles, invocation, or CLI parsing, must stay internal/JDK-based or move to optional integrations outside the core runtime. Phase 28 argument matchers, throwing stubs, and answer callbacks for interface doubles must not introduce bytecode or mocking-library dependencies.
...
- Bootstrap hook execution must remain an explicit Java class/classloader mechanism and must not add ServiceLoader hook discovery, script engines, package scanning, integrated compilation, dependency resolution, or runtime libraries.
...
- Configured bootstrap hooks run immediately before examples, with top-level hooks before selected-suite hooks and duplicates preserved.
...
- Dry-run pending generation/update exits before execution and must not write reports; profile compatibility violations and bootstrap execution failures also exit before report writing.
