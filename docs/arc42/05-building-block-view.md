...
| Release/CI verification assets | `scripts/verify-all.sh`, `.github/workflows/ci.yml` | Provide non-disruptive aggregate local and GitHub Actions verification. Root `mvn verify` remains core-only; standalone Maven plugin, Gradle plugin, and JUnit Platform engine verification run explicitly after installing the current core snapshot. No publishing, signing, secrets, or mandatory Maven multi-module conversion is introduced. |
