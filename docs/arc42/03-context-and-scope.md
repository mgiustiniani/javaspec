...
| CI system | Runs specs, supplies compiled classpaths when needed, consumes exit codes or optional JUnit Platform events, may collect JSON and/or JUnit XML-compatible runner reports with stable ids/source metadata where available, and can run the configured Java 8/11/17/21/25 core matrix plus Java 21 aggregate verification workflow. Phase 20 and Phase 21 have no remote CI success claim in the current evidence. |
