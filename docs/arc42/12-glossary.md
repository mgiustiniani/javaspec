...
| Report | Optional machine-readable output written by `run`, currently JSON via `--report` / `--report-file` or config JSON destination aliases, and JUnit XML-compatible XML via `--junit-xml` / `--junit-xml-file` or config JUnit XML destination aliases. |
