...
Phase 37 provides a standalone optional bytecode doubles adapter at `javaspec-bytecode-doubles/`, packaging `org.javaspec:javaspec-bytecode-doubles:0.1.0-SNAPSHOT`. It is intentionally outside the root Maven reactor and carries ByteBuddy 1.14.18 only in the adapter artifact.

Phase 19 adds deployment-time verification assets: executable `scripts/verify-all.sh` for aggregate local release verification and `.github/workflows/ci.yml` for GitHub Actions. Phase 20 adds release-readiness assets: executable `scripts/check-version-alignment.sh`, `CHANGELOG.md`, `RELEASING.md`, the confirmed MIT `LICENSE`, MIT license and maintainer metadata, Maven `release-artifacts` profiles for local sources/javadocs, Gradle source/javadoc jar readiness, and safe URL/SCM/GitHub Issues metadata. Phase 21 adds adoption-time assets: standalone examples under `examples/`, `scripts/verify-examples.sh`, JSON report schema docs, and golden report examples. Phase 22 updates those report/schema/golden assets for explicit skipped/pending semantics. Phase 23 adds runtime diagnostics for classpath/execution availability without changing artifact packaging or adding compilation. Phase 24 adds configuration-level report destinations without changing artifact packaging, report schemas, or writers. Phase 25 adds JDK ServiceLoader discovery for external run formatter/extension providers without changing artifact packaging, report schemas/content, integrated compilation, dependencies, or publishing; Phase 32 adds activation/formatter controls where implemented. Phase 26 adds target-profile enforcement before generation/update writes without changing artifact packaging, report schemas/content, integrated compilation, dependencies, publishing, or optional adapter architecture. Phase 27 adds bootstrap hook execution before examples, and Phase 33 adds ServiceLoader-discovered hook providers, without changing artifact packaging, report schemas/content, dependencies, publishing, script engine support, package scanning, dependency resolution, or optional adapter architecture. Phase 28 strengthens interface doubles without changing artifact packaging, report schemas/content, integrated compilation, dependencies, publishing, generated assets, examples, or optional adapter architecture. Phase 29 adds opt-in CLI source/spec compilation through the current JDK `javax.tools.JavaCompiler`; Phase 34 extends explicit opt-in compilation to programmatic, Maven, and Gradle paths without changing default behavior, report schemas/content, dependencies, publishing, config keys, JUnit Platform engine behavior, dependency resolution, incremental caches, forked `javac`, or source-level/release management. Phases 30-36 resolve known limitations without core runtime dependency additions. Phase 37 adds optional bytecode concrete-class doubles in a standalone adapter without changing core packaging. These assets do not change runtime packaging, do not publish artifacts, do not sign artifacts, do not add secrets, do not add portal publication/credentials, and do not convert the repository to a Maven multi-module reactor.
...
| Environment | Usage |
|---|---|
| Developer workstation | Run `describe` and `run`, review generated source, enforce target profiles before generation/update writes, optionally compile source/spec trees for CLI runs with `--compile` / `--compile-output <dir>` or for programmatic/Maven/Gradle runs through explicit settings when the current JDK provides `javax.tools.JavaCompiler`, execute configured bootstrap hooks before examples when hook classes are compiled on the run classloader/classpath, execute already-compiled specs, select built-in or ServiceLoader-discovered formatter names, inspect execution-availability diagnostics when compiled classes or dependencies are unavailable, write JSON or JUnit XML-compatible reports with stable ids, pending counts, and source metadata where available when requested by CLI or config destinations, inspect standalone adoption examples, and optionally add `javaspec-bytecode-doubles` for non-final concrete-class doubles. |
| Build tool or IDE classpath | Supplies compiled production/spec classes to the reflection runner through the process classpath, explicit CLI classpath entries, optional Maven plugin test classpath integration, optional Gradle plugin test source set runtime classpath integration, optional JUnit Platform engine runtime classpath integration, or programmatic invocation classloaders. For CLI and Gradle runs, the effective run classloader can also supply ServiceLoader formatter/extension providers. Default runs remain classpath-based; CLI/programmatic/Maven/Gradle entry points can explicitly opt into current-JDK compilation before bootstrap/examples, and Phase 23 diagnostics report when discovered specs/examples are unavailable to the supplied classloader. |
| CI pipeline | Runs root `mvn verify`, dependency audits, optional standalone Maven/Gradle/JUnit Platform engine verification, `scripts/check-version-alignment.sh`, `scripts/verify-all.sh`, `scripts/verify-examples.sh`, optional local source/javadoc artifact checks, optional `javaspec run --dry-run`, optional explicit classpath runs, optional CLI/programmatic/Maven/Gradle source/spec compilation on a JDK, optional `javaspec run --report <file>` / `--junit-xml <file>` or equivalent config report destinations, optional bootstrap hook classes on CLI/Maven/Gradle run classpaths, optional ServiceLoader formatter jars on CLI/Gradle run classpaths, profile compatibility, compilation, and bootstrap failures before reports, and consumes exit codes, stable report identifiers/source metadata/pending counts, execution-availability diagnostics, or JUnit Platform engine events when that optional engine is selected. The GitHub Actions workflow currently defines a Java 8/11/17/21/25 core matrix and a Java 21 full-verification job through `scripts/verify-all.sh`, including adapter and examples checks by default unless explicitly skipped, with no publishing or secrets; Phase 20/21/22 remote success for HEAD `5088e96` on `develop` is user-/maintainer-confirmed. |
...
Standalone optional bytecode doubles adapter verification installs the current core first, then verifies the adapter POM:
...
`javaspec run` discovers source files but executes examples only when compiled spec classes are available on the effective classloader, selected explicit classloader, or CLI compile-output-first classloader. Default CLI runs and adapters remain classpath-based. Phase 23 diagnostics make availability failures explicit; Phase 29 provides only an opt-in CLI compiler step and does not repair classpaths.

Consequences for deployment:

- A source-only spec tree can be discovered but its examples are marked `SKIPPED` with execution-availability reasons unless a CLI run opts into successful `--compile`.
- Users should compile the project, run javaspec from a launcher/classpath that includes compiled spec and production classes, or use CLI `--compile` / `--compile-output <dir>` when they expect source-only CLI execution.
- CLI users can pass already compiled directories, dependency archives, explicit bootstrap hook classes/jars, and external formatter/extension provider jars with `--classpath <path-list>` or a UTF-8 `--classpath-file <file>`; path-list separators use `File.pathSeparator`. With `--compile`, explicit CLI classpath entries are compiler dependency entries after the output directory and before current process `java.class.path`, and execution puts compile output before explicit entries.
- Programmatic callers can supply a classloader and bootstrap hook class names through `JavaspecInvocation` and can call `RunDiagnostics.executionAvailabilityLines(RunResult)` for deterministic availability lines.
...
- The optional Maven plugin uses Maven test dependency resolution and the Maven test classpath to invoke the canonical runner during its `javaspec:run` goal; configured bootstrap hook classes must be available there, and when availability diagnostics exist, it logs `javaspec:` warnings and the Maven test classpath element count.
- The optional Gradle plugin uses the configured Gradle classpath and, when Java plugin source sets are present, defaults `javaspecRun` to the `test` source set runtime classpath and `testClasses` dependency; bootstrap hook classes and external formatter/extension provider jars can be present on that run classpath, and when availability diagnostics exist, it logs `javaspec:` warnings and the Gradle classpath element count.
