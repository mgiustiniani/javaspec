...
- CLI users can pass already compiled directories, dependency archives, explicit bootstrap hook classes/jars, and external formatter/extension provider jars with `--classpath <path-list>` or a UTF-8 `--classpath-file <file>`; path-list separators use `File.pathSeparator`.
