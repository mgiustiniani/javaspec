...
The hook mechanism must run against the same compiled-class view used for examples. javaspec still does not compile source/spec files itself, does not scan packages, and does not add a plugin framework or dependency resolver. Failures must be clear and must stop before examples and report writing so partially initialized runs do not produce misleading results.
...
For CLI `javaspec run --config`, bootstrap execution happens after discovery, profile enforcement, generation/update decisions, and any successful generation/update work, but before example execution and before JSON/JUnit XML-compatible report writing. If discovery finds no specs, the CLI does not execute hooks. A bootstrap failure exits `64`, starts diagnostics with `Error: Bootstrap execution failed`, and writes no reports.
...
Phase 27 deliberately does not add ServiceLoader-based bootstrap discovery, script engines, integrated source/spec compilation, package scanning, dependency resolution, or third-party runtime dependencies. Hook classes must already be compiled and available on the effective run classloader/classpath supplied by the CLI, build tool, or host invocation.
...
- Hooks must be ordinary compiled Java classes visible to the run classloader; javaspec does not compile, scan packages, resolve dependencies, or load scripts for them.
