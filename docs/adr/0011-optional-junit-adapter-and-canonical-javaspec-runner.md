...
Users need better project integration for local development and CI: a no-JUnit invocation path, build-tool classpath handling, CI-friendly reports, Maven and Gradle integration, and optional IDE/CI integration through the JUnit Platform. At the same time, JUnit must not become required for core javaspec usage. Phase 15 implements the standalone optional Maven plugin adapter, Phase 16 implements the standalone optional Gradle plugin adapter, and Phase 17 implements the standalone optional JUnit Platform engine adapter.
...
- The core invocation API must be stable enough for adapters, which increases compatibility pressure on runner results, JUnit Platform unique-id mapping, test identifiers, and report schemas.
