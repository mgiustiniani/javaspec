...
- Phase 25 implemented ServiceLoader external formatter/extension discovery for CLI and Gradle run classloaders. `JavaspecExtensionLoader` loads built-in formatters first, then `RunFormatter`, `JavaspecExtension`, and alias `Extension` providers from the effective run classloader; duplicate extension implementation classes are configured once. Phase 32 later added configuration-driven extension activation and adapter formatter controls where implemented; package scanning, plugin lookup, report schema/content changes, and runtime dependency additions remain out of scope.
...
- Phase 27 implemented bootstrap hook execution before examples. Configured top-level hooks run before selected-suite hooks, order and duplicates are preserved, hook classes load from the effective run classloader/classpath, and hooks receive immutable `BootstrapContext` data. Phase 33 later added deterministic ServiceLoader-discovered `BootstrapHook` providers after explicit hooks. CLI bootstrap failures exit `64` with `Error: Bootstrap execution failed`, skip examples, and write no reports. Bootstrap does not add script engines, package scanning, dependency resolution, automatic classpath repair, or runtime dependencies.
- Phase 28 implemented stronger interface doubles. `ArgumentMatcher`, `ArgumentMatchers`, matcher aliases, argument-constrained stub priority, throwing stubs, and answer callbacks are available while preserving ordinary-interface-only JDK dynamic proxies, Java 8 compatibility, and zero runtime dependencies in core. Phase 37 later added optional non-final concrete-class doubles in a standalone ByteBuddy adapter; final/static/constructor mocking remains unsupported.
...
- Known limitations: default runs remain classpath/reflection based; source-only or otherwise unavailable spec classes are skipped/not executable until compiled classes are present on the effective classloader, selected explicit classloader, adapter classpath, JUnit Platform runtime classpath, or an entry point opts into successful current-JDK compilation where supported. Compilation is explicit and `javax.tools.JavaCompiler` based; it does not add default compilation, config keys, dependency resolution, incremental caches, forked `javac`, source-level/release management, report schema changes, or runtime dependencies. The optional Maven plugin, Gradle plugin, JUnit Platform engine, and bytecode doubles adapter are standalone artifacts not verified by root `mvn verify` alone; use `scripts/verify-all.sh` or explicit standalone verification commands after installing the current core snapshot. Phase 21/37 examples are standalone consumer projects, not root modules, and currently require local snapshot installation rather than public artifact resolution. `scripts/verify-all.sh` includes standalone examples by default, with explicit opt-outs through `JAVASPEC_SKIP_EXAMPLES=1`, `JAVASPEC_SKIP_GRADLE_EXAMPLE=1`, or `JAVASPEC_SKIP_BYTECODE_DOUBLES_EXAMPLE=1`. Public publication remains intentionally postponed: the MIT `LICENSE` and confirmed maintainer metadata are resolved, but GPG signing, Central Portal publication, Gradle Plugin Portal publication/credentials, final release version/tag, and final publish approval remain unresolved and unimplemented. Target-profile enforcement is conservative and source/generation-scoped rather than compiler-grade integrated compilation. ServiceLoader formatter/extension and bootstrap discovery are classpath-based; package scanning, plugin lookup, scripts, dependency resolution, and automatic classpath repair remain unimplemented. JSON reporting remains schemaVersion 1 with additive identifier/source, pending, and metadata/properties fields; JUnit XML-compatible reporting remains intentionally minimal with Phase 35 metadata/properties and skipped/pending `<skipped>` mapping. Generic `Iterable` empty/count checks are now bounded. Existing sealed-interface source updates are now supported source-preservingly. Core doubles remain ordinary-interface-only JDK proxies; optional non-final concrete-class doubles require `javaspec-bytecode-doubles`, and final/static/constructor mocking remains unsupported.
...
- The original numbered roadmap is complete through Phase 18. Phase 19 through Phase 37 are post-roadmap increments: Phase 19 adds aggregate release/CI verification, Phase 20 adds release-readiness scaffolding without public publishing, Phase 21 adds standalone adoption examples plus report schema/golden documentation, Phase 22 adds explicit skipped/pending semantics, Phase 23 adds execution-availability diagnostics, Phase 24 adds configuration-level report destinations, Phase 25 adds ServiceLoader formatter/extension discovery, Phase 26 adds target-profile enforcement before generation/update writes, Phase 27 adds bootstrap hook execution, Phase 28 strengthens interface doubles, Phase 29 adds CLI opt-in source/spec compilation, Phases 30-36 resolve deferred known limitations, and Phase 37 adds the optional bytecode doubles adapter. Future feature work should be tracked as new roadmap or backlog items and must not imply that every conceivable IDE/CI, adoption, or release/publication feature is already implemented.
...
8. Known limitations remain: default execution paths are classpath/reflection based and execute only classloader-available compiled spec classes, while CLI/programmatic/Maven/Gradle entry points can explicitly opt into current-JDK source/spec compilation; lifecycle support is limited to bootstrap hooks plus optional public no-arg `let()`/`letGo()`; source parsing/generation uses Java 8-compatible heuristics rather than a full Java parser; generated post-Java-8 source forms still require an appropriate JDK to compile.
...
- Generic `Iterable` empty checks use a single `hasNext()` peek, and `shouldHaveCount(expected)` iterates at most `expected + 1` elements; one-shot iterables may be advanced by bounded checks, but infinite iterables no longer hang these checks.
...
| Phase 25 ServiceLoader formatter/extension discovery | README, user manual, ARC42 sections 5-11, ADR 0018, this plan | Implemented and locally verified: CLI and Gradle load built-ins first and then JDK ServiceLoader providers for `RunFormatter`, `JavaspecExtension`, and alias `Extension` from their effective run classloaders; duplicate extension implementations are configured once; invalid formatter/provider/extension loading diagnostics are explicit. Phase 32 later resolves configuration-driven activation and adapter formatter parity where implemented; package scanning, plugin lookup, report schema/content changes, publishing changes, and runtime dependency additions remain out of scope. |
| Phase 26/36 target-profile enforcement | README, user manual, ARC42 sections 5-11, ADR 0019, ADR 0023, this plan | Implemented and locally verified: `ProfileEnforcement`, `ProfileEnforcementReport`, and `ProfileViolation` enforce the effective profile after discovery and before generation/update writes, prompts, execution, or reports. Enforcement rejects incompatible described type kinds, resolvable generated method signature API owners, and Phase 36 relationship type references while ignoring unknown/ambiguous types to avoid false positives; it remains conservative source/generation enforcement, not compiler-grade integrated compilation. |
| Phase 27/33 bootstrap hook execution and discovery | README, user manual, ARC42 sections 5-11, ADR 0020, ADR 0023, this plan | Implemented and locally verified: configured top-level hook class names run before selected-suite hooks, order and duplicates are preserved, ServiceLoader-discovered `BootstrapHook` providers run afterward in deterministic order, hooks load from the effective run classloader/classpath, and hooks receive immutable `BootstrapContext` data immediately before examples. CLI no-spec runs skip hooks; bootstrap failures exit before examples/reports with clear diagnostics. Scripts, package scanning, dependency resolution, automatic classpath repair, and runtime dependencies remain out of scope. |
| Phase 28 stronger interface doubles | README, user manual, ARC42 section 5, ADR 0021, this plan | Implemented and locally verified: interface doubles support argument matchers, matcher-constrained stub priority, throwing stubs, answer callbacks, and `Doubles`/`ObjectBehavior` aliases while preserving ordinary-interface-only JDK dynamic proxies, Java 8 compatibility, and zero runtime dependencies in core. Phase 37 separately provides optional non-final concrete-class doubles in a standalone adapter. |
| Phase 29/34 opt-in source/spec compilation | README, user manual, ARC42 sections 5-11, ADR 0022, ADR 0023, this plan | Implemented and locally verified: CLI `javaspec run --compile`, programmatic `JavaspecInvocation.withCompilation(...)`, Maven `javaspec.compile` / `javaspec.compileOutput` / `javaspec.compileOutputDirectory`, and Gradle `compile` / `javaspec.compile` plus `compileOutput` / `javaspec.compileOutput` provide explicit current-JDK `javax.tools.JavaCompiler` compilation. Defaults remain classpath-based; no-spec and dry-run paths skip compilation; compiler-unavailable requests exit/fail clearly; compile failures stop before bootstrap/examples/reports. No config keys, dependency resolution, incremental cache, forked `javac`, source-level/release management, report schema change, or runtime dependency was added. |
...
- Treat bootstrap hooks as explicit configured Java hook class names followed by ServiceLoader-discovered `BootstrapHook` providers; all hooks execute only when compiled classes are available on the effective run classloader/classpath. Do not add script engines, package scanning, dependency resolution, automatic classpath repair, or runtime dependencies.
...
- Keep built-in formatter rendering behind the zero-dependency `RunFormatter` contract and deterministic `RunFormatterRegistry`; keep external formatter/extension discovery classloader-scoped and ServiceLoader-based, with configured extension activation where implemented and no plugin lookup, package scanning, or automatic classpath repair.
...
- Treat the Phase 11 extension API plus ServiceLoader loading and explicit extension activation as formatter/extension registration only; do not imply package scanning, plugin repository lookup, automatic classpath repair, or broader plugin activation.
...
- Treat default runs as classpath reflection execution paths; source-only or unavailable spec classes are skipped/not executable until compiled classes are present. CLI `--compile` / `--compile-output <dir>`, programmatic `withCompilation(...)`, and Maven/Gradle compile settings are explicit current-JDK compiler steps only and do not add default compilation, config keys, dependency resolution, incremental caches, forked `javac`, source-level/release management, JUnit Platform compilation, or runtime dependencies.
...
- Keep core doubles interface-only and JDK-proxy based; Phase 28 argument matchers, throwing stubs, and answer callbacks stay inside this boundary. Phase 37 authorizes non-final concrete-class doubles only through the standalone optional bytecode adapter; unsupported double targets are rejected with clear diagnostics.
...
- Document that generic `Iterable` empty checks are single-peek and count checks are bounded to at most `expected + 1` elements; one-shot iterables may still be advanced by the bounded check.
- Insert generated class/final-class/sealed-class/enum/record method bodies, interface method declarations, annotation elements, and sealed-interface root/nested permitted implementation updates source-preservingly where supported, with confirmation or documented non-interactive behavior; use Java default returns where generated method bodies are required.
...
- Generic `Iterable` empty checks are single-peek and count checks are bounded to `expected + 1` elements.
...
- Argument matchers, throwing stubs, and answer callbacks are implemented in Phase 28. Phase 37 adds optional non-final concrete-class doubles outside the core. Ordered sequences, broader side-effect stubbing, and any final/static/constructor mocking remain future work outside the current core boundary.
...
- Existing sealed-interface source updates are implemented in Phase 31 with root declarations and matching nested permitted implementation bodies inserted source-preservingly.
...
11. External extension loading was not part of Phase 11. Phase 25 implements JDK `ServiceLoader` discovery for external formatter/extension providers, and Phase 32 adds configuration-driven activation plus adapter formatter controls where implemented; classpath/package scanning, plugin lookup, and automatic classpath repair remain out of scope.
...
- Formatter extension contracts are public and programmatic, Phase 25 adds ServiceLoader discovery, and Phase 32 adds configuration-driven extension activation plus adapter formatter controls where implemented. Package scanning, plugin lookup, and automatic classpath repair remain future work.
...
5. Kept programmatic invocation and adapter execution classpath-based at this phase. Phase 29 later adds a CLI opt-in compiler step, and Phase 34 later adds programmatic/Maven/Gradle opt-in compilation; without explicit compilation, explicit classpath entries and adapter-supplied classpaths must point to already compiled classes or archives.
...
7. Did not add package scanning, plugin lookup, report schema/content changes, integrated compilation, runtime dependencies, or publishing changes; Phase 32 later adds configuration-driven activation and adapter formatter controls where implemented.
...
8. Did not add script engines, package scanning, dependency resolution, runtime dependencies, Java 8 compatibility exceptions, report schema changes, or optional adapter architecture changes; Phase 33 later adds ServiceLoader hook discovery after explicit hooks.
...
- README, CHANGELOG, user manual, examples, ARC42, PLAN, and test-report synchronization for the whole program is completed in the finalization documentation phase after Phase 37.

### Excluded from the program

Public publication is explicitly excluded from Phases 30-37, as recorded in ADR 0023:

- **Public publication remains postponed** — GPG signing keys, Central Portal credentials, Gradle Plugin Portal credentials, the final release version/tag, and final publish approval can only be supplied by the maintainer and are not resolvable by code.

Concrete-class doubles are no longer excluded from the resolution program: the maintainer selected the standalone optional bytecode adapter artifact (ADR 0024), implemented as Phase 37 while preserving the zero-runtime-dependency core.
...
4. Examples and documentation are completed in the finalization documentation phase, including `examples/bytecode-doubles-basic/`.
...
This finalization pass synchronized README, CHANGELOG, examples documentation, user manual navigation, ARC42 sections, PLAN, and the test report after Phase 37. Public publication remains intentionally postponed and out of scope.
