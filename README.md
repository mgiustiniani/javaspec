...
Latest Phase 22 verification: targeted changed tests passed with 78 tests; root `mvn -q test` passed with 399 tests; root `mvn -q verify` passed; root runtime dependency audit showed no runtime dependencies beyond `org.javaspec:javaspec`; `mvn -q -DskipTests install` passed; standalone Maven plugin `verify` passed with 13 tests and runtime core-only dependency; standalone JUnit Platform engine `verify` passed with 13 tests and isolated JUnit Platform runtime dependencies; standalone Gradle plugin `clean test build` passed with 12 tests and Java 8 obsolete source/target warnings only; Gradle runtimeClasspath contained only `org.javaspec:javaspec`; `scripts/verify-examples.sh` and `scripts/verify-all.sh` passed with Gradle 8.8. No production Java changes were made by the tester, no blockers were reported, and no Phase 22 remote CI success is claimed. The GitHub Actions workflow is configured for a Java 8/11/17/21/25 core matrix and a Java 21 full-verification job; Phase 19 remote GitHub Actions success for HEAD `4d30e63` on `develop` was user-/maintainer-confirmed, while Phase 20, Phase 21, and Phase 22 have local verification only in the recorded evidence. See [`docs/test-report.md`](docs/test-report.md).
...
6. Render human-readable failures and, when requested, write JSON and/or JUnit XML-compatible runner reports with stable exit codes, stable ids, pending counts, and source file/line metadata where available.
