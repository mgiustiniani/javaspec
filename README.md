...
8. Optionally compile source/spec trees for CLI runs with `--compile` before bootstrap and examples.
9. Execute configured bootstrap hooks immediately before examples when hooks are compiled on the run classloader/classpath.
...
- [`docs/arc42/05-building-block-view.md`](docs/arc42/05-building-block-view.md) — building-block notes, including the generation boundary, CLI run controls, CLI compilation boundary, invocation API, optional Maven/Gradle/JUnit Platform adapters, formatter/reporting/extension contracts, configuration model, profile catalog, profile enforcement compatibility boundary, bootstrap hook lifecycle boundary, and stronger interface doubles boundary.
- [`docs/arc42/06-runtime-view.md`](docs/arc42/06-runtime-view.md) — implemented runtime scenarios for describe, run, execution, generation, doubles, reporting, explicit classpath and opt-in CLI compilation, programmatic invocation, optional Maven/Gradle/JUnit Platform execution, profile enforcement, profile probes, bootstrap execution, and stronger interface doubles.
...
- [`docs/arc42/09-architecture-decisions.md`](docs/arc42/09-architecture-decisions.md) — ADR index, including ADR 0021 for stronger interface doubles and ADR 0022 for opt-in CLI source/spec compilation.
