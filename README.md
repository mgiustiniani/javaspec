...
Latest Phase 21 verification: no core production/test Java files changed; `git diff --check`, whitespace/EOF checks, script syntax checks, executable-bit checks, schema/golden JSON parsing, golden XML parsing, JSON Schema validation, standalone examples verification, and full aggregate `JAVASPEC_GRADLE_BIN=/tmp/gradle-8.8/bin/gradle scripts/verify-all.sh` passed locally. The aggregate output included `==> Standalone examples verification` and `PASS: standalone examples verification completed.` Core tests through `verify-all` remained 386 tests with 0 failures/errors/skips; Maven plugin, JUnit Platform engine, and Gradle plugin checks remained passing; the JUnit Platform example reported 1 passing test; no publish/deploy/signing commands were run. The GitHub Actions workflow is configured for a Java 8/11/17/21/25 core matrix and a Java 21 full-verification job; Phase 19 remote GitHub Actions success for HEAD `4d30e63` on `develop` was user-/maintainer-confirmed, but Phase 20 and Phase 21 have local verification only in the recorded evidence. See [`docs/test-report.md`](docs/test-report.md).
...
- [`docs/examples/reports/`](docs/examples/reports/) — golden passing JSON and JUnit XML-compatible report examples.
- [`docs/test-report.md`](docs/test-report.md) — Phase 12 compatibility matrix plus Phase 14, Phase 15, Phase 16, Phase 17, Phase 18, Phase 19, Phase 20, and Phase 21 verification results.
...
- [`docs/arc42/10-quality-requirements.md`](docs/arc42/10-quality-requirements.md) — quality requirements with Phase 12, Phase 14, Phase 15, Phase 16, Phase 17, Phase 18, Phase 19, Phase 20, and Phase 21 evidence.
